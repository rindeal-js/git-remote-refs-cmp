...
(async () => {
  ...
  const pretty = PinoPretty({
    ...
    customPrettifiers: {
      ...
      level: (_logLevel, _k, log) => LEVELS[log.level as number].emoji || '❓',
    ...
    messageFormat: (log, messageKey, _lL, { colors }) => {
      ...
      const color = LEVELS[log.level as number].colorizer || 'white'
      // @ts-expect-error sdfsd
      return colors[color](log[messageKey] as string)
    }
  })
  ...
  const logDiff = (diff: GitLsRemoteRefDiff | null) => {
    if (diff) {
